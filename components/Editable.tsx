...
const EditMediaIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /> </svg> );
...
const Editable: React.FC<EditableProps> = ({ path, as: Component = 'span', className, multiline = false, label, type = 'text', children, render }) => {
    ...
    const handleBlur = () => {
        if (inputRef.current) handleUpdate(inputRef.current.value);
        setIsEditing(false);
    };
    ...
    if (type === 'media') {
        return (
            <div className="relative group">
                {children}
                <button onClick={() => openMediaModal(path)} className="absolute inset-0 flex items-center justify-center bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    if (isEditing) {
        ...
        return (
            <div className="relative w-full">
                ...
                <InputComponent ref={inputRef} defaultValue={value} onBlur={handleBlur} onKeyDown={handleKeyDown} className={`bg-blue-900/50 text-white p-1 border border-blue-500 rounded-md outline-none w-full resize-none ${className}`} rows={multiline ? 4 : undefined} />
            </div>
        );
    }
    ...
    return (
        <Component onClick={() => setIsEditing(true)} className={`outline-none transition-all duration-300 cursor-pointer hover:outline-2 hover:outline-dashed hover:outline-blue-500 p-1 -m-1 rounded-md ${className}`}>
            {displayContent()}
        </Component>
    );
};
